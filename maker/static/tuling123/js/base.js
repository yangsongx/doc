...
jq().ready( function() {
	...
	jq.dialog = function (settings) {
		...
		var $dialogOverlay = jq("#dialogOverlay" + dialogId);
		var $dialog = jq("#" + dialogId);
		var $dialogTitle = jq("#dialogTitle" + dialogId);
		var $dialogClose = jq("#dialogClose" + dialogId);
		var $dialogOk = jq("#dialogOk" + dialogId);
		var $dialogCancel = jq("#dialogCancel" + dialogId);
		
		$dialog.css({"width": settings.width, "height": settings.height, "margin-left": - parseInt(settings.width / 2), "z-index": dialogZIndex ++});
		
		if(!window.XMLHttpRequest) {
			var $dialogIframe = jq("#dialogIframe" + dialogId);
		...
		$dialogTitle.mousedown(function (event) {
			...
			jq(document).bind("mousemove", function(event) {
	...
	jq.closeDialog = function (dialogId) {
		var $dialogOverlay = jq("#dialogOverlay" + dialogId);
		var $dialog = jq("#" + dialogId);
