...
function QiniuJsSDK() {
    ...
    this.uploader = function(op) {
        ...
        var getUpToken = function() {
            //if (!op.uptoken) {
            var beforetime = getCookie("beforetime");
            var nowtime = (new Date()).getTime();
            var date3=nowtime-beforetime;  //时间差的毫秒数
            var hours=Math.floor(date3/(3600*1000));

            var cookietoken = getCookie("qiniu_token");
            if(hours>=1 || cookietoken == '')                                                  ////yuan
            {
                ...
                ajax.onreadystatechange = function() {
                    if (ajax.readyState === 4 && ajax.status === 200) {
                        ...
                        setCookie("qiniu_token", res.uptoken);                        ////yuan
                        var beforetime = (new Date()).getTime();
                        setCookie("beforetime",beforetime);
                    }
                };
                ajax.send();
            } else {
                //that.token = op.uptoken;
                that.token = cookietoken;
                //setCookie("qiniu_token", op.uptoken,0.1);                        ////yuan
            }
        };
        ...
        uploader.bind('Init', function(up, params) {
            //var cookietoken = getCookie("qiniu_token");                            ////yuan
            //if(cookietoken == '')                                                  ////yuan
            //{
                getUpToken();
            //}
    ...
    this.exif = function(key) {
        ...
        //var url = this.getUrl(key) + '?exif';                                          yuan
        var url = this.getUrl(key);                                                    //yuan
        var xhr = this.createAjax();
        var info;
        var that = this;
        xhr.open('GET', url, false);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                //info = that.parseJSON(xhr.responseText); //Disable exif temporarily
                info = "";
            }
        };
        xhr.send();
