...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressBarBox.addClass('info').css('width', '480px').css('margin','20px 0px 0px 0px');
        var progressBarWrapper = $("<div/>");
        progressBarWrapper.addClass("progress progress-striped").css('width', '100%');
...
FileProgress.prototype.reset = function() {
    ...
    this.fileProgressWrapper.find('td .progress .progress-bar-info').attr('aria-valuenow', 0).width('100%').find('span').text('');
    this.appear();
};
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var progressBarChunkTr = $('<tr class="chunk-status-tr"><td colspan=1></td></tr>');
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    this.fileProgressWrapper.find('.status').text("已上传: " + size + "，上传速度： " + formatSpeed + "/s (" + percentage +"%)");
...
//    this.fileProgressWrapper.hide();
    ...
    if (!isImg) {
        ...
        var ii = Qiniu.imageInfo(res.key);
        cropImage(url, res.key, ii.width, ii.height);
